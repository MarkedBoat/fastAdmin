...
let __ElementExt = function (tag) {
    ...
    /**
     * 和上面 setIndexHandler  设置句柄及索引  类似，只不过，不需要单独挑出变量进行设置，在addNode 和  addNodes 时，加上
     * @param init_handle_name
     * @returns {__ElementExt}
     */
    self.setInitHanderName = function (init_handle_name) {
        self.initHandleName = init_handle_name;
        return self;
    };
    ...
    self.addNodes = function (nodes) {
        for (let i in nodes) {
            ...
            if (typeof node === 'string') {
                ...
                if (node.initHandleName.length > 0) {
                    self.getClosestRootElement()[node.initHandleName] = node;
                }
     ...
     * @returns {__ElementExt}
     */
    self.getClosestRootElement = function () {
        if (self.isRoot === true) {
            console.log(self);
            return self;
        } else {
            if (self.boss) {
                return self.boss.getClosestRootElement();
            } else {
                return self;
            }
        }
    ...
    /**
     * 声明这是一个isRoot element
     * <br>!!!!!!!!!  声明越早!!!越早越好
     * @returns {__ElementExt}
     */
    self.declareThisARootElement = function () {
        self.isRoot = true;
        return self;
    };
