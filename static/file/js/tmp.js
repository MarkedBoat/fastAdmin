...
(function () {
    ...
    js.addEventListener('load', function () {
        ...
        if (kl.id('issue_notes')) {
            ...
            var input_file_charset = new Emt('select');
            var select_page = new Emt('select');
            input_file_charset.add(new Option('UTF8', 'UTF8'));
            input_file_charset.add(new Option('GBK', 'GBK'));
            input_file_charset.add(new Option('GB2312', 'GB2312'));
            ...
            var strs = [];
            book.addNodes([btn_file, input_file_line, input_file_charset, select_page]);
            ...
            if (typeof tmp[0] !== 'undefined') {
                ...
                for (var i = 0; i < as.length; i++) {
                    if (as[i].href.indexOf('.jpg') === -1 && as[i].href.indexOf('.png') === -1) {
                        console.log(i, as[i]);
                        continue;
                    }
                    console.log('图片', as[i].href, as[i].href.indexOf('.jpg'), as[i].href.indexOf('.png'));
            ...
            var filereader_bg = new FileReader();
            var filereader_bg_pos = 0;
            var page_num = 0;
            ...
            btn_file.addEventListener('change', function () {
                ...
                filereader_bg.file = tmp;
                filereader_bg.onload = function (evt) {
                    // console.log(evt.target.result);
                    getPages(evt.target.result);
                };
            ...
            function initPages() {
                window.readSi = setInterval(function () {
                    //  console.log(filereader_bg_pos, file_block_size);
                    filereader_bg.readAsText(filereader_bg.file.slice(filereader_bg_pos, (filereader_bg_pos + file_block_size)), input_file_charset.value);
                    filereader_bg_pos = filereader_bg_pos + file_block_size;
                }, 5);
            }

            function getPages(text) {
                if (text.length < 100) {
                    window.clearInterval(window.readSi);
                    return false;
                }
                var strs = text.split("\n");
                strs.forEach(function (str) {
                    var matched = str.match(/第(.*)?章 /);
                    if (matched === null) {
                        return false;
                    }
                    page_infos.push({title: str, pos: filereader_bg_pos});
                    // console.log('第' + matched[1] + '章',str,page_infos);
                    select_page.add(new Option(str, (filereader_bg_pos / file_block_size).toString()));
                });
            ...
            function getText(num) {
                ...
                var file_start = (page_num - 1) * file_block_size - 40;
                if (end < 0) {
                    file_start = 0;
                    end = file_block_size;
                }
                filereder.readAsText(filereder.file.slice(file_start, end), input_file_charset.value);
