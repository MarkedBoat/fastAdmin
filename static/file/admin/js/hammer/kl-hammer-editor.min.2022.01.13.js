    ...
    opt_input.ui_ele.addEventListener('paste', function (event) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (event.clipboardData.items[i].type.match(/^image\//)) {
                ...
                handle_form.input_title_ele.value = '截图';
                handle_form.input_detail_ele.value = '截图';
                ...
                console.log(handle_uploader);
                handle_uploader.loadInputFile(file_obj);
                handle_form.process_ele.reset();
                handle_modal.show();
            }
        }
    });
