/**
 *
 * @param input_opt  {
 * csrf:'string',
 * title:'title',
 * }
 * @returns {{hide: hide, show: show}}
 */
let hammerBootstarpDatagrid = function (input_opt) {
    let handle_this = {
        __doc: {
            ele: '将网格 填到哪个地方',
            title: '网格标题',
            requestDataFun: 'fun,里面根据 本handle提供的信息 进行请求 指定数据  ，参数:  1.事件(filter变动事件/点击跳页/点击排序/点击goto按钮)，   2.{filter_data:{},page:{},sort:{}  } 3:回调函数，这个由本handle提供的 handle_this.reloadDataRows，实现代码负责回调，用于更新数据',
            preCondtion: '预设查询条件',
        },
        ele: {}
    };
    for (var k in handle_this.__doc) {
        if (typeof input_opt[k] !== 'undefined') {
            handle_this[k] = input_opt[k];
        }
    }


    handle_this.ele.root = new Emt('table', 'class="table table-bordered table-striped table-hover"');
    handle_this.ele.root.thead = handle_this.ele.root.createTHead();
    handle_this.ele.root.tbody = handle_this.ele.root.createTBody();
    handle_this.ele.root.tfoot = handle_this.ele.root.createTFoot();
    handle_this.ele.root.tcaption = handle_this.ele.root.createCaption();

    handle_this.addTr = function (table_tpart, opt) {
        let last_tr = table_tpart.insertRow();
        if (typeof opt === 'object') {
            last_tr.setAttrs(opt);
        }
        last_tr.addTd = function (cell_key, opt2) {
            let last_td = last_tr.insertCell();
            if (typeof cell_key === 'string') {
                last_tr['cell_' + cell_key] = last_td;
            }
            last_td.setPros = function (configs) {
                for (var attr in configs)
                    last_td[attr] = configs[attr];
                return last_td;
            };

            if (typeof opt === 'object') {
                last_td.setPros(opt2);
            }

            last_td.addNode = function () {
                for (var i = 0; i < arguments.length; i++) {
                    if (typeof arguments[i] !== 'string') {
                        last_td.appendChild(arguments[i]);
                        if (typeof arguments[i + 1] === 'string') {
                            if (arguments[i + 1]) self[arguments[i + 1]] = arguments[i];
                        }
                    }
                }
                return last_td;
            };
            last_td.addNodes = function (nodes) {
                for (var i in nodes) {
                    var node = nodes[i];
                    if (typeof node === 'string') {
                        last_td.innerHTML += node;
                    } else {
                        last_td.appendChild(node);
                    }
                }
                return last_td;
            };

            return last_td;
        }
        return last_tr;
    }


    handle_this.tr_top = handle_this.addTr(handle_this.ele.root.thead);
    handle_this.tr_title = handle_this.addTr(handle_this.ele.root.thead);
    handle_this.tr_filter = handle_this.addTr(handle_this.ele.root.thead);
    handle_this.tr_foot = handle_this.addTr(handle_this.ele.root.tfoot);


    let input_hide_sort_column_key = new Emt('input').setPros({type: 'hidden'});
    let input_hide_sort_type = new Emt('input').setPros({type: 'hidden'});
    let input_hide_page_index = new Emt('input').setPros({type: 'hidden', value: 1});
    let input_hide_page_size = new Emt('input').setPros({type: 'hidden', value: handle_this.page_size || 20});
    if (handle_this.preCondtion && handle_this.preCondtion.sort && handle_this.preCondtion.sort.key && handle_this.preCondtion.sort.type) {
        input_hide_sort_column_key.value = handle_this.preCondtion.sort.key;
        input_hide_sort_type.value = handle_this.preCondtion.sort.type;
    }

    if (handle_this.preCondtion && handle_this.preCondtion.page && handle_this.preCondtion.page.size && handle_this.preCondtion.page.index) {
        input_hide_page_index.value = handle_this.preCondtion.page.index;
        input_hide_page_size.value = handle_this.preCondtion.page.size;
    }


    let btn_submit = new Emt('button').setPros({className: 'btn btn-default', textContent: '=>'});
    handle_this.tr_top.addTd('goto_submit').addNodes([
        btn_submit,
        input_hide_sort_column_key,
        input_hide_sort_type,
    ]);

    handle_this.tr_foot.addTd('page').addNodes([
        input_hide_page_index,
        input_hide_page_size
    ]);


    handle_this.filter_eles = {keys: [], ele_map: {}, btns: []};


    handle_this.requestData = function (event_type) {
        let data = {
            sort: {key: input_hide_sort_column_key.value, type: input_hide_sort_type.value},
            page: {size: input_hide_page_size.value, index: input_hide_page_index.value},
            filter: {}
        };
        handle_this.filter_eles.keys.forEach(function (column_key) {
            let tmp_val = handle_this.filter_eles.ele_map[column_key].getFilterValue();
            if (tmp_val === '#####') return false;
            data.filter[column_key] = tmp_val;
        })
        console.log('event_type', event_type, 'data', data);
        handle_this.requestDataFun(event_type, data, handle_this.reloadDataRows);
    }

    handle_this.getInputData = function () {
        let data = {
            sort: {key: input_hide_sort_column_key.value, type: input_hide_sort_type.value},
            page: {size: input_hide_page_size.value, index: input_hide_page_index.value},
            filter: {}
        };
        handle_this.filter_eles.keys.forEach(function (column_key) {
            let tmp_val = handle_this.filter_eles.ele_map[column_key].getFilterValue();
            if (tmp_val === '#####') return false;
            data.filter[column_key] = tmp_val;
        })
        return data;
    }
    handle_this.gotPage = function () {
        handle_this.requestData('page');
    }
    handle_this.resortFun = function (btn_ele) {
        handle_this.requestData('sort');
    }
    handle_this.filterChange = function (btn_ele) {
        handle_this.requestData('filter');
    }

    btn_submit.addEventListener('click', function () {
        handle_this.requestData('goto');
    })

    handle_this.createSpan = function (opt) {
        return new Emt('span');
    }
    handle_this.createInputText = function (opt) {
        let tmp_input = new Emt('input', 'type="text" ');
        tmp_input.getFilterValue = function () {
            return tmp_input.value;//根据不同opt 做对应的转化
        }
        return tmp_input;
    }


    handle_this.createInputSelect = function (opt, ele_key) {
        let select_ele = new Emt('select', 'type="text" ');
        select_ele.addItem = function (label, val) {
            select_ele.add(new Option(label, val));
        }
        select_ele.addItem('不选择', '___no_select');
        if (opt && opt.list && typeof opt.list.forEach === 'function') {
            opt.list.forEach(function (item) {
                select_ele.addItem(item.label, item.val);
            })
        }
        return select_ele;
    }


    handle_this.data_cell = {keys: [], fun_map: {}};


    /**
     * filterInputElement/filter_ele 必须是本类创建的，不然有些东西怕是监听不到，但是有些列的filter是没必要的，填string 也可以是的
     * title/label 就是 column中文名
     * column_name/column_key 行数据的 列 字段, 用来存放操作按钮的列，也得起个名字
     * isSortable/is_sortable true:可怕徐
     * cellRenderFunction/data_cell_function(td_cell,row_data) 针对数据行的，操作行还有其他东西进行填充，但是数据行就得调相应得方法了，必须接受 单元格(cell) 和 行数据(row_data)作为参数
     */
    handle_this.addColumn = function (input_param) {
        let init_config = input_param;
        let filter_ele = init_config.filterInputElement;
        let column_label = init_config.title;
        let is_sortable = init_config.isSortable;
        let data_cell_function = init_config.cellRenderFunction;
        let column_info = init_config.columnInfo;
        let column_key = init_config.dataAttrKey;

        console.log('filter:', filter_ele, 'label:', column_label, 'column_key:', column_key, 'is_sortable:', is_sortable, 'data_cell_function', data_cell_function);
        //hide_btn_sort  hide_sort_asc hide_sort_desc
        let btn_header;
        if (is_sortable === true) {
            btn_header = new Emt('BUTTON', 'type="button" class="btn-default btn-xs hide_btn_sort"', column_label);
            btn_header.addNodes([
                new Emt('SPAN', 'class="glyphicon glyphicon-arrow-up btn_sort_asc"', ''),
                new Emt('SPAN', 'class="glyphicon glyphicon-arrow-down btn_sort_desc"', '')
            ]);
            btn_header.sort_info = {type: false};
            btn_header.clearSort = function () {
                btn_header.sort_info.type = false;
                btn_header.classList.remove('hide_btn_sort_asc');
                btn_header.classList.remove('hide_btn_sort_desc');
                btn_header.classList.add('hide_btn_sort');
            }
            btn_header.setSortAsc = function () {
                btn_header.sort_info.type = 'asc';
                btn_header.classList.remove('hide_btn_sort_asc');
                btn_header.classList.add('hide_btn_sort_desc');
                btn_header.classList.remove('hide_btn_sort');
            }
            btn_header.setSortDesc = function () {
                btn_header.sort_info.type = 'desc';
                btn_header.classList.add('hide_btn_sort_asc');
                btn_header.classList.remove('hide_btn_sort_desc');
                btn_header.classList.remove('hide_btn_sort');
            }

            handle_this.filter_eles.btns.push(btn_header);
            btn_header.addEventListener('click', function () {
                handle_this.filter_eles.btns.forEach(function (tmp_btn) {
                    if (tmp_btn === btn_header) return false;
                    tmp_btn.clearSort();
                });
                if (btn_header.sort_info.type === 'asc') {
                    btn_header.setSortDesc();
                } else {
                    btn_header.setSortAsc();
                }
                input_hide_sort_column_key.value = column_key;
                input_hide_sort_type.value = btn_header.sort_info.type;
                handle_this.resortFun(this);
            });
        } else {
            btn_header = new Emt('span', 'class="btn-default btn-xs"', column_label);
        }
        if (column_info && column_info.remark) {
            btn_header.setAttribute("title", column_info.remark);
        }
        handle_this.tr_title.addTd(column_key).addNodes([btn_header]);
        if (typeof filter_ele === 'string') {
            handle_this.tr_filter.addTd(column_key).textContent = filter_ele;
        } else {
            filter_ele.classList.add('col-md-12');
            filter_ele.addEventListener('change', function () {
                handle_this.requestData('filter');
            })
            handle_this.tr_filter.addTd(column_key).addNodes([filter_ele]);
        }
        handle_this.data_cell.keys.push(column_key);
        if (typeof data_cell_function !== 'undefined') {
            if (typeof data_cell_function === 'function') {
                handle_this.data_cell.fun_map[column_key] = data_cell_function;
            } else {
                throw  'data_cell_function 不是函数,要么给function,要么不给';
            }
        }

        if (typeof filter_ele.getFilterValue === 'function') {
            handle_this.filter_eles.keys.push(column_key);
            handle_this.filter_eles.ele_map[column_key] = filter_ele;
        }
        if (handle_this.preCondtion && handle_this.preCondtion.attrs && handle_this.preCondtion.attrs[column_key] !== undefined && handle_this.preCondtion.attrs[column_key].length > 0) {
            filter_ele.value = handle_this.preCondtion.attrs[column_key];
        }
        return handle_this;
    }

    handle_this.data_trs = [];
    handle_this.addDataRow = function (row_data) {
        let data_tr = handle_this.addTr(handle_this.ele.root.tbody);
        handle_this.data_trs.push(data_tr);
        data_tr.data_info = row_data;
        handle_this.data_cell.keys.forEach(function (column_key) {
            let data_cell = data_tr.addTd(column_key);
            if (typeof row_data[column_key] === 'undefined') {
                if (typeof handle_this.data_cell.fun_map[column_key] !== 'function') {
                    console.log(row_data, column_key);
                    throw column_key + ',row_data没有这个字段，也没有设置处理function';
                }
                handle_this.data_cell.fun_map[column_key](data_cell, row_data);
            } else {
                if (typeof handle_this.data_cell.fun_map[column_key] === 'function') {
                    handle_this.data_cell.fun_map[column_key](data_cell, row_data);
                } else {
                    data_cell.innerText = row_data[column_key] === null ? '' : row_data[column_key].toString();
                }
            }
        })
    }
    handle_this.clearDataRows = function () {
        let max = handle_this.data_trs.length;
        for (let i = 0; i < max; i++) {
            //console.log(handle_this.data_trs.length);
            handle_this.data_trs[i].remove();
        }
        handle_this.data_trs = [];
    }
    handle_this.pageInfo = {};
    handle_this.createPager = function (cnt) {
        handle_this.pageInfo.all_total = cnt;
        if (typeof handle_this.pageInfo.root_ele === "undefined") {

            handle_this.pageInfo.goto_start = new Emt('a').setPros({href: '#', textContent: '<<', goto: 1});
            handle_this.pageInfo.goto_end = new Emt('a').setPros({href: '#', textContent: '>>', goto: 'end'});
            handle_this.pageInfo.goto_pre = new Emt('a').setPros({href: '#', textContent: '<', goto: 'pre'});
            handle_this.pageInfo.goto_next = new Emt('a').setPros({href: '#', textContent: '>', goto: 'next'});

            handle_this.pageInfo.root_ele = new Emt('ul').setPros({className: 'pagination'}).addNodes([
                new Emt('li').addNodes([
                    handle_this.pageInfo.goto_start,
                    handle_this.pageInfo.goto_pre
                ])
            ]);

            for (let i = 0; i < 5; i++) {
                handle_this.pageInfo['goto_' + i.toString()] = new Emt('a').setPros({href: '#', textContent: '#', goto: i});
                handle_this.pageInfo.root_ele.addNodes([
                    new Emt('li').addNodes([
                        handle_this.pageInfo['goto_' + i.toString()]
                    ])
                ]);
            }

            handle_this.pageInfo.root_ele.addNodes([new Emt('li').addNodes([
                handle_this.pageInfo.goto_next,
                handle_this.pageInfo.goto_end
            ])]);
            handle_this.tr_foot.cell_page.addNodes([handle_this.pageInfo.root_ele]);
            ['start', 'end', 'pre', 'next', 0, 1, 2, 3, 4].forEach(function (ele_name) {
                handle_this.pageInfo['goto_' + ele_name.toString()].addEventListener('click', function () {
                    console.log('goto', ele_name.toString(), this.goto);
                    input_hide_page_index.value = this.goto;
                    handle_this.gotPage();
                })
            });
        }

        handle_this.pageInfo.curr_index = parseInt(input_hide_page_index.value);
        handle_this.pageInfo.page_size = parseInt(input_hide_page_size.value);
        handle_this.pageInfo.page_total = Math.ceil(parseInt(cnt) / handle_this.pageInfo.page_size);

        let min = handle_this.pageInfo.curr_index - 2;
        let max = handle_this.pageInfo.curr_index + 2;
        console.log('curr_index', handle_this.pageInfo.curr_index, 'page_size', handle_this.pageInfo.page_size, 'page_total:', handle_this.pageInfo.page_total, 'cnt', cnt, min, max);
        if (min < 1) {
            diff = 0 - min + 1;
            min = 1;
            max = max + 2;
        }
        console.log(min, max, handle_this.pageInfo);

        let j = 0;
        for (let i = min; i <= max; i++) {
            console.log(i, j);
            if (handle_this.pageInfo['goto_' + j.toString()]) {
                handle_this.pageInfo['goto_' + j.toString()].setPros({textContent: i.toString(), goto: i});
                j = j + 1;
            }
        }
        handle_this.pageInfo.goto_end.goto = handle_this.pageInfo.page_total;
        handle_this.pageInfo.goto_next.goto = handle_this.pageInfo.curr_index + 2;
        handle_this.pageInfo.goto_pre.goto = handle_this.pageInfo.curr_index - 2;

        ['start', 'end', 'pre', 'next', 0, 1, 2, 3, 4].forEach(function (ele_name) {
            let a_ele = handle_this.pageInfo['goto_' + ele_name.toString()];
            a_ele.classList.remove('hide');
            a_ele.parentElement.classList.remove('active');
            if (a_ele.goto < 1) {
                a_ele.goto = 1;
            } else if (a_ele.goto > handle_this.pageInfo.page_total) {
                a_ele.goto = handle_this.pageInfo.page_total;
                if (typeof ele_name === 'number') {
                    a_ele.classList.add('hide');
                }
            }
            if (typeof ele_name === 'number' && a_ele.goto === handle_this.pageInfo.curr_index) {
                a_ele.parentElement.classList.add('active');
            }
        });


    }
    /**
     * 重新载入数据
     * @param row_datas
     */
    handle_this.reloadDataRows = function (row_datas, total_cnt) {
        handle_this.clearDataRows();
        row_datas.forEach(function (row_data) {
            handle_this.addDataRow(row_data);
        });
        handle_this.tr_top.cell_goto_submit.setAttribute('colspan', handle_this.filter_eles.keys.length);
        handle_this.tr_foot.cell_page.setAttribute('colspan', handle_this.filter_eles.keys.length);
        handle_this.createPager(total_cnt);
    }


    if (input_opt.ele) {
        input_opt.ele.appendChild(new Emt('div', 'class="table-responsive" ').addNodes([handle_this.ele.root]));
    }
    return handle_this;

};

