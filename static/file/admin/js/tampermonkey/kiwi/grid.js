...
let HammerTvGridCellElement = function (input_data) {
    ...
    let btn_self = new Emt(init_data.tagName || 'button', init_data.AttrStr || 'type="button"');
...
let HammerTvGrid = function () {
    ...
    self.createTr = function () {
        ...
        tr_self.createTd = function () {
            ...
            td_self.onArrowInput = function (arrow_direction) {
                top.window.kl.log('onArrowInput', arrow_direction);
    ...
    self.loadGridData2UI = function () {
        ...
        self.uiGrid.forEach(function (tds, index_y) {
            tds.forEach(function (td, index_x) {
                self.uiGrid[index_y][index_x].closeCell('loadGridData2UI init');
            });
     ...
     * @return HammerTvGrid
     */
    self.bindGrid = function (direction, hammerGridObject) {
        let tmp_map = {left: 'right', right: 'left', top: 'down', down: 'top',};
        if (tmp_map[direction] === undefined) {
            alert('bindGrid-参数错误' + direction);
            throw 'bindGrid-参数错误';
        }
        self.config.directions[direction] = {isGrid: true, grid: hammerGridObject};
        hammerGridObject.config.directions[tmp_map[direction]] = {isGrid: true, grid: self};
        return self;
    };
...
if (top.window.kl.tvGrid !== true) {
    top.window.kl.tvGrid = true;
            ...
            if ([13, 37, 38, 39, 40].indexOf(whichCode) !== -1) {
                ...
                switch (whichCode) {
                    case 13:
                        ...
                        break;
                    case 37:
                        tmp_ele.uiGridCell.onArrowInput('left');
                        break;
                    case 38:
                        tmp_ele.uiGridCell.onArrowInput('top');
                        break;
                    case 39:
                        tmp_ele.uiGridCell.onArrowInput('right');
                        break;
                    case 40:
                        tmp_ele.uiGridCell.onArrowInput('down');
                        break;
                }
                // e.preventDefault();
                // return false;
