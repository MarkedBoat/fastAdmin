(function () {
        ...
        //插件默认捕捉XHR请求，但是为了减少资源开支，可以通过设置 localStorage 暂停记录,有100ms的延迟
        top.window.kl.kiwiJs.stopRecordXhr = function () {
            localStorage.setItem('record_xhr', 'false');
        };
        top.window.kl.kiwiJs.keepRecordXhr = function () {
            localStorage.setItem('record_xhr', 'true');
        };

        //插件为了防止浏览器被写爆，插件入口设置了保险，会自动清理Storage,如果有website的程序接手，那就需要解除 插件入口保险,不适合在 website/index 调用
        top.window.kl.kiwiJs.shutdowDefaultXhrStorageCleaner = function (flag) {
            top.window.kl.log('shutdowDefaultXhrStorageCleaner kl_kiwi_js_xhr_record_data_default_cleaner ', flag);
            localStorage.setItem('kl_kiwi_js_xhr_record_data_default_cleaner', 'false');
            top.window.kl.log('kl_kiwi_js_xhr_record_data_default_cleaner', localStorage.getItem('kl_kiwi_js_xhr_record_data_default_cleaner'));
                    ...
                    if (is_callback === true) {
                        ...
                        if (tmp_ar.length === 2) {
                            top.window.kl.kiwiJs.captureXhrRecorder.triggers.forEach(function (trigger_info, trigger_index) {
                                if (trigger_info.isClose === false && tmp_ar[0].indexOf(trigger_info.url) === 0) {
                                    trigger_info.fun({
                                        key: key,
                                        url: tmp_ar[0],
                                        text: tmp_ar[1],
        ...
        let root_div = new Emt('div', 'id="kl_kiwi_menu_root" class="big_btn_div" ');
        ...
        let top_grid = new HammerTvGrid().setUIColsNumber(5).setUIRowsNumber(1);
        //   top.window.kl.kiwiJs.golobInfo.currentGrid = top_grid;

        let website_grid = (new HammerTvGrid()).setUIColsNumber(2).setUIRowsNumber(2);
        top.window.kl.kiwiJs.root_div.addNodes([
            top_grid,
            website_grid
        ]);

        top_grid.loadSourceArray(menu_map_top_btns).loadGridData2UI();

        website_grid.loadSourceArray(menu_map_website_btns).loadGridData2UI();
        //top_grid.drawUIGrid();
        //website_grid.drawUIGrid();
        website_grid.bindGrid('top', top_grid);
        top_grid.setIndexHandler(top.window.kl.kiwiJs.root_div, 'top_grid');
        ...
        if (!top.document.getElementById('kl_kiwi_menu_style')) {
            ...
            top.document.body.append(new Emt('style', 'id="kl_kiwi_menu_style"', '' +
                '#kl_kiwi_menu_root{position: fixed; background: #DDD;width: 100%; height: ' + h + 'px; top: 0px; left: 0px; z-index: 99999; font-size: 20px;}' +
                '#kl_kiwi_menu_root button{}' +
                '.big_btn_div button{font-size:20px;}' +
                '#kl_kiwi_menu_root button:focus{border-left:solid #000 0.3em;border-right:solid #000 1em;}' +
                '.hide{display:none;}' +
                '.keep_hide{display:none;}' +
                '.kl_kiwi_play_box{}' +
                '.kl_kiwi_list_box{background: #DDD;width: 100%; min-height:100%;max-height:100%; }' +
                '.kl_kiwi_list_box_title{ background: #DDD;width: 100%; height: 10%; top: 0px; font-size: 20px;}' +
                '.kl_kiwi_list_box_list{ background: #DDD;width: 100%;height:90%;  top: 0px; font-size: 14px;}' +
                ...
                '.kl_kiwi_list_box_fixed{position: fixed; top: 0px; left: 0px; z-index: 999999;background: #DDD;width: 100%;max-height:100%; }' +
                ...
                '.kl_kiwi_menu_root_msg_box_text{position: relative;width: 80%;margin: 0 auto;margin-top: -50px;left: 0;/*background: #FFF;*/ color: #F00;font-size: 2em;font-weight: 900;}' +
