let biInput = function (init_config) {
    console.log(init_config.path + '->' + init_config.key + '->' + 'biInput', init_config);
...
let biObject = function (input_param, db_val) {
    console.log(input_param.path + '->' + 'INIT biObject', {input_param: input_param, db_val: db_val});
    let bi_object = this;
...
let biArray = function (input_param, db_val) {
    console.log(input_param.path + '[]->' + 'INIT　biArray', {input_param: input_param, db_val: db_val});
    ...
    console.log(input_param.path + '[]->' + 'biArray opt', bi_array);
    ...
    bi_array.addElementEle = function (element_db_val) {
        ...
        if (bi_array.opt.elementStruct.input_type === 'object') {
            ...
            console.log(bi_array.opt.path + '->' + 'bi_array.addElementEle: will create biObject ', {config: tmp_config, element_db_val: element_db_val});
            let tmp_bi_object = new biObject(tmp_config, element_db_val);
            ...
            tmp_config.onChanged = bi_array.opt.onChanged;
            console.log(bi_array.opt.path + '->' + 'bi_array.addElementEle： will create biArray ', {config: tmp_config, element_db_val: element_db_val});
            let tmp_bi_array = new biArray(tmp_config, element_db_val);
            ...
            console.log(bi_array.opt.path + '->' + 'bi_array.addElementEle:  will create biInput ', {config: tmp_config, element_db_val: element_db_val});
            let tmp_bi_input = new biInput(tmp_config, element_db_val);
...
let biRow = function (input_param, db_val) {
    ...
    if (bi_row.opt.inputType === 'array') {
        ...
        bi_row.inputEle = new biArray(bi_row.opt, db_val);
        ...
        bi_row.inputEle = new biObject(bi_row.opt, db_val);
        bi_row.inputEle.apiHandle.setVal(db_val);
        row_ele.firstElementChild.addNode(bi_row.inputEle);
        bi_row.getVal = function () {
            return bi_row.inputEle.apiHandle.getVal();
        ...
        bi_row.inputEle = new biInput(bi_row.opt, db_val);
        if (db_val !== undefined) {
            bi_row.inputEle.apiHandle.setVal(db_val);
        }
...
let biStruct = function (struct_data, db_data, callFunction) {
    ...
    let bi_row = new biRow({
        ...
        path: '#',
        onChanged: function (input_ele) {
            console.log('biStruct', {input_ele: input_ele});
            let val = input_ele.apiHandle.getVal();
            val.struct_code = struct_data.struct_code;
            console.log('biStruct input_ele.getVal', val, console.log(JSON.stringify(val, null, 4)));
            console.log("\nval:\n\n\n" + JSON.stringify(val) + "\n\n\n");
            console.log("\nstruct:\n\n\n" + struct_json + "\n\n\n");
