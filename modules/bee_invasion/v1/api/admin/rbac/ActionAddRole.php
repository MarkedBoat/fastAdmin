...
class ActionAddRole extends AdminBaseAction
{
    public function run()
    {
        $id = $this->inputDataBox->tryGetString('id');
        $RbacModel                  = new RbacRoleDao();

        if($id){
            $RbacModel->id    = $id;
            $RbacModel = RbacRoleDao::model()->findByPk($id, false);
            if(!$RbacModel){
            ...
            if($name){
                $RbacModel->role_name    = $name;
            }
            $code = $this->inputDataBox->tryGetString('code');
            if($code){
                $RbacModel->role_code =$code;
            }
