        ...
        if($name == 'menu' || $name == 'role_menu'){
            ...
            switch ($name){
                case 'menu' :
                    ...
                    if($Menu->pid == 0){
                        $RoleMenu = Sys::app()->db('dev')->setText("select * from bi_bg_rbac_menu where pid = :pid")->bindArray(['pid'=>$Menu->id])->queryRow();
                        if($RoleMenu){
                ...
                case 'role_menu':
                    ...
                    if($Menu->pid == 0){
                        //是父级获取子集
                        $sonMenu = Sys::app()->db('dev')->setText("select id from bi_bg_rbac_menu where pid = :pid")->bindArray(['pid'=>$Menu->id])->queryAll();
                        if($sonMenu){
                            ...
                            $RoleMenu = Sys::app()->db('dev')->setText("select * from bi_bg_rbac_role_menu where menu_id in (".$id_str.")")->queryRow();
                            if($RoleMenu){
                                throw new \Exception('该菜单子级仍在使用，请先删除子集数据');
                            }
                        }
