...
class ActionTool extends ActionBase
{


    public function run()
    {
        $cacheName = $_REQUEST['key'];
...
//上传oss图片
    ...
    public function getArea(){
        $items = Sys::app()->db('dev')->setText("select * from bi_area")->queryAll();
        $tree = []; //格式化好的树
    ...
    public function getAreaPro(){
        $items = Sys::app()->db('dev')->setText("select area_code,area_name,if(area_level=1,0,if(area_level=2,province_code,city_code)) parent_id from bi_area  union all
select concat(area_code,'x') area_code,'其他' area_name ,city_code parent_id from bi_area where area_level=2 ORDER BY area_code")->queryAll();
    ...
    public function getAreaName($area_code){
        $name = Sys::app()->db('dev')->setText("select area_name from bi_area where area_code = :area_code")->bindArray(['area_code'=>$area_code])->queryRow();
