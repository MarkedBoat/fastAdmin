...
class ActionHis extends GameBaseAction
{
    ...
    {
        return ['user_id', 'item_code', 'page_index'];// 'page_size',
    }
    ...
    public function getInnerDataUpdateTimeKeys()
    {
        ...
        if ($item_code)
        {
            $list[] = ApiCache::model()->getCacheKey('ChangeFlagUserObject', ['user_id' => $this->user->id]);
    ...
    public function run()
    {
        ...
        $dao = UserObjectHis::model()->addSort('id', 'desc');
