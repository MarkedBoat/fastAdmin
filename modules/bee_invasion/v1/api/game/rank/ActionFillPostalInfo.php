...
class ActionFillPostalInfo extends GameBaseAction
{
    public function run()
    {
        $award_his_id = $this->inputDataBox->getIntNotNull('award_his_id');
        ...
        $award_his_dao = UserRankAwardHis::model()->findByPk($award_his_id, false);
        if (empty($award_his_dao))
        {
            return $this->dispatcher->createInterruption(AdvError::res_not_exist['detail'], '信息不存在', false);
        }

        $his_dao = UserObjectHis::model()->findByPk($award_his_dao->his_id, false);
        if (empty($his_dao))
        {
            return $this->dispatcher->createInterruption(AdvError::data_info_unexpected['detail'], '信息不存在', false);
        }
        ...
        $award_his_dao->award_status = UserObjectHis::stepWaitInfoCheck;
        $award_his_dao->update(false);
