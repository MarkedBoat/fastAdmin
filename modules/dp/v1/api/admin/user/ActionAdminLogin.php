...
//            }
        ...
        $arr = Sys::app()->db('dev')->setText("select * from dp_bg_admin where " . $where)->bindArray($data)->queryRow();
        if (!$arr)
        {
        ...
        else
        {
            ...
            //校验后台权限
            $userNode = Sys::app()->db('dev')->setText("select * from dp_bg_rbac_user_role where user_id = :user_id")->bindArray(['user_id'=>$arr['id']])->queryRow();
            if(!$userNode){
    ...
    public function getToken($uid)
    {
        ...
        $arr                = Sys::app()->db('dev')->setText("insert into dp_bg_admin_token" . $where . "ON DUPLICATE KEY UPDATE user_token = :user_token,expires = :expires ")->bindArray($data)->execute();
        return $token;
    }
