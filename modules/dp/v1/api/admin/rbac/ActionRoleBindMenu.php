...
class ActionRoleBindMenu extends AdminBaseAction
{
    public function run()
    {
        ...
        if($id){
            ...
            if($Menu->pid != 0){
                $RoleMenu = Sys::app()->db('dev')->setText("select * from dp_bg_rbac_role_menu where menu_id = :menu_id and role_id = :role_id")->bindArray(['menu_id'=>$Menu->pid,'role_id'=>$role_id])->queryRow();
