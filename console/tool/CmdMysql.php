        ...
        if ($this->yesOrNoConfirm("是否导出数据，注意选择，\n!!!!!!!!!!小心覆盖已有文件!!!!!", ['export'], "开始导出", "跳过导出"))
        {
            ...
            foreach ($matched_fullnames as $matched_fullname)
            {
                ...
                while (true)
                {
                    ...
                    $got_rows_cnt  += $curr_rows_cnt;
                    $loop_times    += 1;

                    $now_time     = time();
                    $now_date     = date('Y-m-d H:i:s', $now_time);
                    $cost_seconds = $now_time - $last_time;
                    $this->printer->tabEcho("table loop:{$tables_i}/{$tables_cnt} :{$matched_fullname}   min_id:{$min_id} rows cnt:{$table_rows_cnt} rows loop times:{$loop_times}  current rows cnt:{$curr_rows_cnt}   [{$got_rows_cnt}/{$total_rows_cnt}]  [{$now_date}] cost:{$cost_seconds}");
