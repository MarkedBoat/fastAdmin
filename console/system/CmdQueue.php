<?php
    ...
    class CmdQueue extends CmdBase {
        ...
        public function call() {
            echo "\n fuck\n";
            $timeout   = $this->inputDataBox->getIntNotNull('timeLimit');
            $startTime = time();
            ...
            echo "start:{$d1}\n";
            $className = $this->inputDataBox->getStringNotNull('class');
            $object    = new $className;
            $fun       = $this->inputDataBox->getStringNotNull('method');
            $queueName = $this->inputDataBox->getStringNotNull('queue');
            $tab       = str_repeat(' ', 10);
            while (TRUE) {
                echo "{$tab}";
