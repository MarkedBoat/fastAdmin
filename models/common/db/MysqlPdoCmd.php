...
class MysqlPdoCmd
{
    ...
    public function debugInfo($throw_exception = false)
    {
        preg_match_all('/:\w+/', $this->commandText, $ar);
        $bindKeys = array_keys($this->bindData);
        ...
        if ($throw_exception)
        {
            Sys::app()->interruption()->setMsg('debug mysql:')->setCode('mysql_error_exec_error')->setDebugMsg('debug')->setDebugData([
                'sql'  => $this->commandText,
                'bind' => $this->bindData,
                'db'   => $this->getDb(),
                'diff' => ['bindMore' => array_diff($bindKeys, $ar[0]), 'sqlMore' => array_diff($ar[0], $bindKeys)]
            ])->outError();
        }
        else
        {
