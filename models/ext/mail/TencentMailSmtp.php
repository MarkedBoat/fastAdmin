...
namespace models\ext\mail;

/**
 * 一个简单的PHP SMTP 发送邮件类
 */
class TencentMailSmtp
{
    /**
    ...
    public function sendMail($title, $content, $attachment_file = '')
    {
        $mail = new static();
        $mail->setServer("smtp.exmail.qq.com", "admin@mail.markedboat.com", "xxxxx", 465, true); //参数1（qq邮箱使用smtp.qq.com，qq企业邮箱使用smtp.exmail.qq.com），参数2（邮箱登陆账号），参数3（邮箱登陆密码，也有可能是独立密码，就是开启pop3/smtp时的授权码），参数4（默认25，腾云服务器屏蔽25端口，所以用的465），参数5（是否开启ssl，用465就得开启）//$mail->setServer("XXXXX", "joffe@XXXXX", "XXXXX", 465, true);
        $mail->setFrom("admin@mail.markedboat.com");                                          //发送者邮箱
        $mail->setReceiver("yangjinlong@mail.markedboat.com");                                //接收者邮箱
        if ($attachment_file)
        {
            $mail->addAttachment($attachment_file);                                                          //Attachment 附件，不用可注释
        }
        $mail->setMail($title, $content);                                                            //标题和内容
